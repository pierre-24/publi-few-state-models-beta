...
\abstract{A series of valence bond charge-transfer (VB-$n$CT) models ($n \in [1,4]$) is investigated to assess the influence of molecular symmetry and electronic structure parameters, particularly the ground-state charge-transfer character, quantified by the VB-CT mixing parameter $m_{CT}$, on second-order nonlinear optical (NLO) response, $\beta$. The analysis focuses on quantities accessible via second harmonic scattering (SHS).  Among the symmetries considered, $D_{3h}$ and $C_{3v}$ (VB-3CT, with large $\theta$) yield the highest $\beta_{SHS}$ values when $m_{CT}$ is large, followed by $T_d$ (VB-4CT). Other symmetries result in comparatively weaker responses. Maximizing the charge-transfer dipole moment $\mu_{CT}$ and minimizing the ratio $\xi = T/t$, are also identified as critical factors for enhancing second-order NLO performance.}
\keywords{Nonlinear optics, second harmonic generation, few state models}
...
In the VB-$n$CT model, the dipole matrix elements $M_{\nu\xi} = \braket{\phi_\nu | \hat{\mu} | \phi_\xi}$ are given as follows \cite{luValenceBondChargeTransferModel1994}:
\begin{align}
