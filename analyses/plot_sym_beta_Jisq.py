...
from few_state.properties import beta_J1sq, beta_J3sq
...
parser.add_argument('--ymax', default=0.01, type=float)
...
plot_walks(
    ax1,
    walks,
    lambda s: beta_J1sq(s.response_tensor((1, 1), frequency=0)),
    ['$m_{{CT}}={:.2f}$'.format(m_CT) for m_CT in args.m_CTs],
    (0, args.ymax),
    '$|\\beta_{J=1}|^2\\times\\mu_{CT}^{-6} \\times t^4$',
)

plot_walks(
    ax2,
    walks,
    lambda s: beta_J3sq(s.response_tensor((1, 1), frequency=0)),
    None,
    (0, 40 * args.ymax),
    '$|\\beta_{J=3}|^2\\times\\mu_{CT}^{-6} \\times t^4$',
)
